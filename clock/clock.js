  ...
  async function updateWeather() {
    try {
...

      {
        ...
        const hrs = sunriseTime.getHours().toString().padStart(2, "0");
        const mins = sunriseTime.getMinutes().toString().padStart(2, "0");
        sunriseEl.querySelector(".hours .tens .value").textContent = hrs[0];
        sunriseEl.querySelector(".hours .ones .value").textContent = hrs[1];
        sunriseEl.querySelector(".minutes .tens .value").textContent = mins[0];
        sunriseEl.querySelector(".minutes .ones .value").textContent = mins[1];
      }

      {
        const sunsetEl = weatherElement.querySelector(".sunset");
        const sunsetTime = new Date(now.sunset);
        const hrs = sunsetTime.getHours().toString().padStart(2, "0");
        const mins = sunsetTime.getMinutes().toString().padStart(2, "0");
        sunsetEl.querySelector(".hours .tens .value").textContent = hrs[0];
        sunsetEl.querySelector(".hours .ones .value").textContent = hrs[1];
        sunsetEl.querySelector(".minutes .tens .value").textContent = mins[0];
        sunsetEl.querySelector(".minutes .ones .value").textContent = mins[1];
      }

      {
        const updatedEl = weatherElement.querySelector(".updated .time");
        const updatedTime = new Date();
        const updatedString = updatedTime.toLocaleTimeString("sl-SI", {
          hour: "2-digit",
          minute: "2-digit",
        });
        updatedEl.textContent = `${updatedString}`;
        weatherElement.style.outline = "";
      }
    } catch (error) {
      weatherElement.style.outline = "1px solid #f00";
    } finally {
